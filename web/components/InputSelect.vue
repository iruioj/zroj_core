...
<template>
  <div class="group relative">
    <div class="flex pl-2 border rounded border-slate-400 group-hover:border-brand cursor-pointer text-slate-500"
      @click="onToggle">
      <div class="w-full select-none py-1" :class="!selected && 'text-secondary'">
        {{ selected?.title || placeholder || "" }}
      </div>
      <ExpandMoreIcon class="bg-white/0 h-8 w-7 p-1 fill-secondary group-hover:fill-brand transition-transform"
    ...
    <div v-if="showSelect" class="z-10 absolute w-full">
      <ul class="bg-back border border-black/10 shadow-xl w-full">
        <li v-for="item in items" :key="item.value"
          ...
          :class="selected?.value === item.value && 'text-brand'" @click="onSelect(item.value)">
          {{ item.title }}
        </li>
      </ul>
    </div>
